/**
 * CHABAD WILLIAMSBURG VIRGINIA EVENTS - JavaScript Functionality
...
(function() {
    ...
    if (isRegistrationPage) {
        ...
        function styleInfoCards() {
            ...
            if (!infoSection || infoSection.classList.contains('cwv-info-styled')) return;
            ...
            newWrapper.className = 'cwv-info-wrapper';
            newWrapper.style.cssText = 'display: block !important;';

            if (sectionTitle) {
                var titleEl = document.createElement('div');
                titleEl.className = 'cwv-info-section-title';
            ...
            cardsGrid.className = 'cwv-info-cards-grid';

            if (locationData) {
                var locCard = document.createElement('div');
                locCard.className = 'cwv-info-card';
                locCard.innerHTML = '<div class="cwv-info-icon">' + singleEventIcons.location + '</div>' +
                    '<div class="cwv-info-text"><span class="cwv-card-label">LOCATION</span>' +
                    '<div class="cwv-info-content">' + locationData + '</div></div>';
                if (locationHref) {
            ...
            if (dateData) {
                ...
                dateCard.className = 'cwv-info-card';
                dateCard.innerHTML = '<div class="cwv-info-icon">' + singleEventIcons.calendar + '</div>' +
                    '<div class="cwv-info-text"><span class="cwv-card-label">DATE & TIME</span>' +
                    '<div class="cwv-info-content">' + dateData + '</div></div>';
                if (dateHref) {
            ...
            infoSection.classList.add('cwv-info-styled');
        }
        ...
        function stylePricingSection() {
            var registerBody = document.querySelector('#RegisterBody');
            if (!registerBody || registerBody.classList.contains('cwv-pricing-styled')) return;
            ...
            if (pricingItems.length === 0 || pricingItems.length > 20) {
                registerBody.classList.add('cwv-pricing-styled');
                return;
            }

            var pricingContainer = document.createElement('div');
            pricingContainer.className = 'cwv-pricing-container';

            var titleEl = document.createElement('div');
            titleEl.className = 'cwv-pricing-title';
            titleEl.textContent = 'Ticket Pricing';
            pricingContainer.appendChild(titleEl);

            pricingItems.forEach(function(item) {
                var itemEl = document.createElement('div');
                itemEl.className = 'cwv-pricing-item';

                var labelEl = document.createElement('span');
                labelEl.className = 'cwv-pricing-label';
                if (item.isVip) {
                    var badge = document.createElement('span');
                    badge.className = 'cwv-vip-badge';
                ...
                amountEl.className = 'cwv-pricing-amount';
            ...
            if (otherText.length > 0) {
                otherText.forEach(function(txt) {
                    ...
                    p.className = 'cwv-sponsorship-text';
                    p.textContent = txt;
            ...
            if (sponsorshipLevels.length > 0) {
                ...
                levelsContainer.className = 'cwv-sponsorship-levels';
                sponsorshipLevels.forEach(function(level) {
                    var pill = document.createElement('span');
                    pill.className = 'cwv-sponsorship-level';
        ...
        function initCategoryPreservationFix() {
            ...
            if (Events._cwvCategoryFixApplied) return;
            Events._cwvCategoryFixApplied = true;
        ...
        if (form) {
            var formObserver = new MutationObserver(function() {
                clearTimeout(window.cwvFormValidationDebounce);
                window.cwvFormValidationDebounce = setTimeout(function() {
                    forceFormInputStyles();
                }, 100);
            });
        ...
        console.log("✅ Chabad Williamsburg Virginia Single Event styling applied!");
        return;
    }
    ...
    function parseEventContent(bottomPaddingEl) {
        ...
        var cityStatePattern = /\b[A-Z][a-z]+,?\s+(AZ|Arizona|CA|California|NV|Nevada|VA|Virginia)\s+\d{5}/i;
        var pricePattern = /\$\d+/;
    ...
    function transformEvent(eventEl) {
        if (eventEl.classList.contains("cwv-modernized")) return;
        ...
        newHTML += '<div class="cwv-event-header">';
        if (imageSrc) {
            newHTML += '<div class="cwv-event-image"><img src="' + imageSrc + '" alt="' + title + '"></div>';
        }
        newHTML += '<div class="cwv-header-content">';
        newHTML += '<span class="cwv-event-category">Event</span>';
        newHTML += '<h2 class="cwv-event-title">' + title + '</h2>';
        newHTML += '</div></div>';

        newHTML += '<div class="cwv-event-body">';

        if (parsed.description.length > 0) {
            newHTML += '<p class="cwv-event-description">' + parsed.description.join(" ") + '</p>';
        }

        newHTML += '<div class="cwv-event-info">';

        if (parsed.dateTime.length > 0) {
            newHTML += '<div class="cwv-info-row">';
            newHTML += '<div class="cwv-info-icon">' + icons.clock + '</div>';
            newHTML += '<div class="cwv-info-content">';
            newHTML += '<span class="cwv-info-label">When</span>';
            newHTML += '<span class="cwv-info-value">' + parsed.dateTime.join("<br>") + '</span>';
            newHTML += '</div></div>';
        }

        if (parsed.location.length > 0) {
            newHTML += '<div class="cwv-info-row">';
            newHTML += '<div class="cwv-info-icon">' + icons.location + '</div>';
            newHTML += '<div class="cwv-info-content">';
            newHTML += '<span class="cwv-info-label">Where</span>';
            newHTML += '<span class="cwv-info-value">' + parsed.location.join("<br>") + '</span>';
            newHTML += '</div></div>';
        }
        ...
        if (pricing.length > 0) {
            newHTML += '<div class="cwv-pricing-grid">';
            pricing.forEach(function(p) {
                newHTML += '<div class="cwv-price-item">';
                newHTML += '<span class="cwv-price-label">' + p.label + '</span>';
                newHTML += '<span class="cwv-price-value">' + p.amount + '</span>';
                newHTML += '</div>';
            });
            newHTML += '</div>';
        }
        ...
        newHTML += '<div class="cwv-event-footer">';
        newHTML += '<div class="cwv-date-display">';
        if (dateInfo.month && dateInfo.day) {
            newHTML += '<div class="cwv-date-box">';
            newHTML += '<span class="cwv-date-month">' + dateInfo.month + '</span>';
            newHTML += '<span class="cwv-date-day">' + dateInfo.day + '</span>';
            newHTML += '</div>';
        }
        if (dateInfo.text) {
            newHTML += '<span class="cwv-date-text">' + dateInfo.text + '</span>';
        }
        newHTML += '</div>';

        newHTML += '<div class="cwv-footer-actions">';
        newHTML += '<label class="cwv-login-checkbox" id="cwv-login-label-placeholder"><span>Login before registering</span></label>';
        newHTML += '<a href="' + registerUrl + '" class="cwv-register-btn">Register ' + icons.arrow + '</a>';
        newHTML += '</div>';
        newHTML += '</div>';

        eventEl.classList.add("cwv-modernized");
        eventEl.insertAdjacentHTML("afterbegin", newHTML);

        var placeholderLabel = eventEl.querySelector('#cwv-login-label-placeholder');
        var originalCheckbox = eventEl.querySelector('input[id$="LoginFirst"]');

        if (placeholderLabel) {
            var newCheckbox = document.createElement('input');
            newCheckbox.type = 'checkbox';
            newCheckbox.id = 'cwv-login-checkbox-' + Math.random().toString(36).substring(2, 11);
            placeholderLabel.insertBefore(newCheckbox, placeholderLabel.firstChild);
            placeholderLabel.removeAttribute('id');

            var registerButton = eventEl.querySelector('.cwv-register-btn');
    ...
    function wrapEventsInGrid() {
        ...
        if (document.querySelector('.cwv-events-grid')) return;

        var grid = document.createElement('div');
        grid.className = 'cwv-events-grid';
    ...
    function stylePageTitle() {
        ...
        pageTitle.classList.add('cwv-styled-title');

        if (!document.getElementById('cwv-title-accent')) {
            var accent = document.createElement('div');
            accent.id = 'cwv-title-accent';
    ...
    function moveLoginNotice() {
        ...
        var eventsGrid = document.querySelector('.cwv-events-grid');
