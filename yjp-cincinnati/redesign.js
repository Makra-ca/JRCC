...
(function() {
    ...
    function buildEventsSection(events) {
        ...
        if (events.length === 0) {
            ...
            events.slice(0, 4).forEach(event => {
                ...
                title.style.cssText = `font-size:24px;color:${COLORS.navyBlue};margin-bottom:12px;font-weight:700;`;
    ...
    async function fetchInstagramPosts() {
        // Fetch Instagram post URLs from Google Sheet
        // SETUP INSTRUCTIONS:
        // 1. Create a Google Sheet with column A = Instagram post URLs
        // 2. File → Share → Publish to web → Select "Comma-separated values (.csv)" → Publish
        // 3. Copy the CSV URL and paste it below

        const GOOGLE_SHEET_CSV_URL = 'https://docs.google.com/spreadsheets/d/e/2PACX-1vQVF4z8xBFPw3IQgZCCbyBDOFydVReff3uiKEgcLXsQFcz1MO2ktI0De_Aa7QVbMvVi2DFAnUQI6e_x/pub?output=csv';

        if (!GOOGLE_SHEET_CSV_URL) {
            console.log('YJP: No Google Sheet configured for Instagram posts');
            return [];
        }
        ...
        try {
            ...
            // Parse CSV - each line is a URL
            const posts = csv
                .split('\n')
                .map(line => line.trim())
                .filter(line => line.includes('instagram.com/p/') || line.includes('instagram.com/reel/'))
                .slice(0, 6); // Max 6 posts

            console.log('YJP: Fetched', posts.length, 'Instagram posts from Google Sheet');
            return posts;
        } catch (error) {
    ...
    function buildSocialSection(instagramPosts = []) {
        // Instagram section with posts from Google Sheet
        ...
        feedContainer.style.cssText = 'margin:40px 0;';
        ...
        if (INSTAGRAM_POSTS.length > 0) {
            ...
            INSTAGRAM_POSTS.forEach(postUrl => {
                const embedWrapper = document.createElement('div');
                embedWrapper.style.cssText = 'max-width:400px;width:100%;';

                // Create blockquote embed
            ...
            // Placeholder when no posts configured
            feedContainer.innerHTML = `
                <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:16px;max-width:600px;margin:0 auto;">
                    ${[1,2,3].map(() => `
                        <a href="https://instagram.com/yjp_cincinnati" target="_blank" style="display:block;aspect-ratio:1;background:linear-gradient(45deg, ${COLORS.lightGray}, #e8e8e8);border-radius:12px;display:flex;align-items:center;justify-content:center;transition:transform 0.3s;text-decoration:none;" onmouseover="this.style.transform='scale(1.03)'" onmouseout="this.style.transform='scale(1)'">
                ...
                <p style="color:${COLORS.textMuted};font-size:14px;margin-top:20px;">
